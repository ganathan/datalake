...
# ----------------------------------------------------------
def invoke_lambda(target_lambda_arn, target_gluejb_lambda_arn, source_bucket, source_name, target_lambda_role_arn, crawler_name, path, domain_name, table_name, value, partition_values, replicate, db_name, db_schema):
...
# --------------------------------------------------------------------
def invoke_controller_stepfunction(account_id, glue_db_name, commands, controller, state_machine_arn):
    try:
        params = {
...
# ---------------------------------------------------------------------
# IGet config details from the client s3 bucket
# ---------------------------------------------------------------------
        ...
        try:
            for record in json.loads(event['Records'][0]['body'])['Records']:
                ...
                if is_dot_folder_object != -1 and is_access_control != -1:
                    ...
                    resp = invoke_controller_stepfunction(account_id, glue_db_name, commands, controller, state_machine_arn=event_controller_stepfn_arn)
                    ...
                    if extension == 'xml' or extension == 'xls' or extension == 'xlsx' or extension == 'md':
                        ...
                        (account_id, region, entity, replicate, db_name, db_schema) = get_config_details(source_bucket)
